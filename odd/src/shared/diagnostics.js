...
( function () {
	...
	function parseAppHtmlForProbe( html, baseUrl ) {
		var out = {
			...
			hasRoot: false,
			hasImportmap: false,
			hasFetchBootstrap: html.indexOf( 'oddout_apps_iframe_fetch_bootstrap' ) !== -1,
			hasDiagnosticsBootstrap: html.indexOf( 'oddout_apps_iframe_diagnostics_bootstrap' ) !== -1,
			moduleScripts: [],
			styles: [],
			imports: {},
		};
	...
	function collectMarkdown() {
		...
		var lines = [
			...
			'- catalog raw/effective bundles: `' + [
				p.systemHealth.catalog && p.systemHealth.catalog.raw_bundle_count || 0,
				p.systemHealth.catalog && p.systemHealth.catalog.effective_bundle_count || 0,
			].join( '/' ) + '`',
			'- catalog registry hash: `' + ( p.systemHealth.catalog && p.systemHealth.catalog.registry_sha256 || '' ) + '`',
			'- catalog signature: `' + ( p.systemHealth.catalog && p.systemHealth.catalog.signature_status || '' ) + '`',
			'- catalog stale age: `' + ( p.systemHealth.catalog && p.systemHealth.catalog.stale_age || 0 ) + '` seconds',
			'- catalog rollback count: `' + ( p.systemHealth.catalog && p.systemHealth.catalog.rollback_count || 0 ) + '`',
