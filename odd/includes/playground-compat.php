 ...
 */
function odd_playground_compat_remove_dm_pwa_head_tags() {
	if ( ! function_exists( 'odd_is_playground_host' ) || ! odd_is_playground_host() ) {
		return;
	}
...
add_action( 'plugins_loaded', 'odd_playground_compat_remove_dm_pwa_head_tags', 30 );

add_filter(
	'desktop_mode_shell_config',
	static function ( $config ) {
		if ( ! function_exists( 'odd_is_playground_host' ) || ! odd_is_playground_host() || ! is_array( $config ) ) {
			return $config;
		}
