...
| Dock filters: `desktop_mode_dock_items`, `desktop_mode_dock_item`, `desktop_mode_dock_item_multi`, `desktop_mode_dock_placement`, `desktop_mode_arrange_menu_items` | supported | ODD uses native dock filters for placement/config hooks only. Icon sets intentionally do not re-skin rail, dock, taskbar, or system tile artwork. |
