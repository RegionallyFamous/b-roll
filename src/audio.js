...
( function () {
	...
	if ( window.__bRoll.audio && window.__bRoll.audio._installed ) return;
	...
	function enable() {
		...
		return navigator.mediaDevices.getUserMedia( { audio: true, video: false } )
			.then( function ( s ) {
				...
				state.error      = null;
				window.dispatchEvent( new CustomEvent( 'b-roll:audio-change' ) );
				return true;
			} )
			.catch( function ( err ) {
				state.enabled = false;
